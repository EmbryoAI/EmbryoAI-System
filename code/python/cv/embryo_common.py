...
def outer_edge(img, thresh=3500, resol=3.75):
    ...
    img2 = cv2.adaptiveThreshold(
        img1, 5, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 3, 1
    )
    # 拉普拉斯算法求图像边缘
    img3 = cv2.Laplacian(img2, cv2.CV_64F, ksize=1).astype(np.uint8)
    # 求图像所有闭合轮廓
    _, cnt, _ = cv2.findContours(img3, cv2.RETR_TREE, cv2.CHAIN_APPROX_TC89_L1)
...
def cell_edge(img, thresh=2500, resol=3.75):
    ...
    if len(avail_zone) == 2:
        return (
            (avail_zone[0], avail_area[0], np.sqrt(avail_area[0] / pi) * 2),
