...
def find_embryo(img, minSize=(400, 400), maxSize=None):
    ...
    cascade_file = getdefault(
        conf, "CASCADE_TEMPLATE", "embryo_cascade.xml"
    )  # 配置的cascade分类器文件名
    cascade = cv2.CascadeClassifier(
        os.path.dirname(__file__) + os.path.sep + cascade_file
    )
    rects, _, confidences = cascade.detectMultiScale3(
        img, minSize=minSize, maxSize=maxSize, outputRejectLevels=True
    )
    if len(rects) < 1:
        # 没有找到胚胎目标
        return None
