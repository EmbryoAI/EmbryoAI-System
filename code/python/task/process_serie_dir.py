...
def process_serie(path, serie, dish_info):
    ...
    for c in wells:
        ...
        # 某个孔所有图像的完整路径列表
        well_image_files = [
            f"{serie_path}{i:05d}.jpg"
            for i in range(wells[c].fileStart, wells[c].fileEnd)
        ]
        # 获得该孔的最清晰图像
        try:
        ...
        if sharpest:
            ...
            # 保存缩略图
            cv2.imwrite(
                focus_path + focus_file, img_out, [int(cv2.IMWRITE_JPEG_QUALITY), 50]
            )
            outer_cnt, outer_area, outer_diameter = outer_edge(img)
            if (
                isinstance(outer_cnt, np.ndarray)
                and outer_cnt.tolist()
                and len(outer_cnt)
            ):
            ...
            if (
                serie_info.outerDiameter
                and serie_info.innerDiameter
                and serie_info.outerDiameter > serie_info.innerDiameter
            ):
                serie_info.zonaThickness = (
                    serie_info.outerDiameter - serie_info.innerDiameter
                ) * 0.425
