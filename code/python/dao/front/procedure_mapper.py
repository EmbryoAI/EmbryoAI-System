...
def deleteProcedure(params):
    try:
        sql = text("update t_procedure set del_flag=:delFlag " "where id=:id")
        logUtils.info(sql)
...
def getEmbryoFateCount(medicalRecordNo, embryoFateId):
    try:
        ...
        return db.session.execute(
            sql, {"medicalRecordNo": medicalRecordNo, "embryoFateId": embryoFateId}
        ).fetchone()[0]
    except Exception as e:
        raise DatabaseError("根据主键ID查询病历时异常", e.message, e)
        return None
    ...
    try:
        ...
        for dish_code in dishCodeList:
            ...
            dish = (
                ...
                .filter(Dish.incubatorId == incubatorId, Dish.dishCode == code)
                .one_or_none()
            )
            if not dish:
                dishId = uuid()
            ...
            for i in embryos:
                ...
                cell = (
                    ...
                    .filter(Cell.dishId == dishId, Cell.cellCode == cellCode)
                    .one_or_none()
                )
                if not cell:
                    cellId = uuid()
...
def updateCollect(procedureId, capStartTime, capEndtime):
    try:
        ...
        db.session.execute(
            sql,
            {
                "capStartTime": capStartTime,
                "capEndtime": capEndtime,
