...
def insertMilestone(milestone, milestoneData, procedure, cap_start_time):
    try:
        ...
        embryoScore = updateEmbryoScore(milestone.embryoId)
        # 修改胚胎表总分
        db.session.commit()
...
def countMilestoneScore(milestone, milestoneData, procedure, cap_start_time):
    ...
    timeValue = get_serie_time_hours(
        insemiTime, cap_start_time, milestone.milestoneTime
    )
    # 计算时间
    engine.declare(
        Fact(stage=stageDict.dictValue, condition="time", value=str(timeValue))
    )
    # 计算节点
    if stageDict.dictValue == "PN":
        engine.declare(
            Fact(
                stage=stageDict.dictValue,
                condition=pnIdDict.dictClass,
                value=pnIdDict.dictValue,
            )
        )
    elif (
        stageDict.dictValue == "2C"
        or stageDict.dictValue == "3C"
        or stageDict.dictValue == "4C"
        or stageDict.dictValue == "5C"
        or stageDict.dictValue == "8C"
    ):
        engine.declare(
            Fact(
                stage=stageDict.dictValue,
                condition=cellDict.dictClass,
                value=cellDict.dictValue,
            )
        )
        engine.declare(
            Fact(
                stage=stageDict.dictValue,
                condition=evenDict.dictClass,
                value=evenDict.dictValue,
            )
        )
        if (
            stageDict.dictValue == "3C"
            or stageDict.dictValue == "4C"
            or stageDict.dictValue == "5C"
            or stageDict.dictValue == "8C"
        ):
            engine.declare(
                Fact(
                    stage=stageDict.dictValue,
                    condition=fragmentDict.dictClass,
                    value=fragmentDict.dictValue,
                )
            )
        if stageDict.dictValue == "8C":
            engine.declare(
                Fact(
                    stage=stageDict.dictValue,
                    condition=gradeDict.dictClass,
                    value=gradeDict.dictValue,
                )
            )
    engine.run()
...
def queryMilestoneList(embryoId):
    ...
    try:
        milestoneList = (
            ...
            .filter(Milestone.embryoId == embryoId)
            .order_by("milestone_time")
            .all()
        )
    except Exception as e:
