...
def queryByProcedureIdAndDishId(procedureId, dishId):
    try:
        procedureDish = db.session.query(ProcedureDish).filter(ProcedureDish.procedureId 
            == procedureId, ProcedureDish.dishId == dishId).one_or_none()
...
def queryByImagePathAndDishId(imagePath, dishId):
    try:
        sql = text("""
            ...
            AND tp.del_flag = 0
        """)
        procedureDish = db.session.execute(sql, {'imagePath': imagePath, 'dishId': dishId})\
            .one_or_none()
        return procedureDish
...
def queryEmbryoId(imagePath, dishId, cellCode):
    try:
        ...
        data = db.session.execute(sql, {
            'imagePath': imagePath, 'dishId': dishId,'cellCode': cellCode
            }).fetchone()
