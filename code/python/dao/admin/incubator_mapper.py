...
def queryIncubatorList(page, limit, filters):
    try:
        pagination = (
            Incubator.query.filter_by(**filters)
            .order_by(Incubator.createTime.desc())
...
def updateIncubator(params):
    try:
        sql = text(
            "update sys_incubator set incubator_code=:incubatorCode,incubator_brand=:incubatorBrand, incubator_type=:incubatorType"
...
def deleteIncubator(params):
    try:
        sql = text("update sys_incubator set del_flag=:delFlag " "where id=:id")
        logUtils.info(sql)
        db.session.execute(sql, params)
        db.session.commit()
    except Exception as e:
