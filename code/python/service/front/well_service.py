...
def queryWellList(procedureId, dishId, cellCode):
    try:
        ...
        if not os.path.isdir(path):
            return 500, f"采集目录下查无对应皿目录:{path}"
        ...
        if not os.path.isfile(jsonPath):
            return 500, f"皿目录下查无对应皿JSON文件:{jsonPath}"
        ...
        if not current_cell:
            500, f"查询当前孔信息异常dishId:{dishId}cellCode{cellCode}"
...
def getWellVideo(agrs):
    try:
        ...
        video_name = (
            video_path + os.path.sep + pd.imagePath + f"_DISH{dishCode}_{well_id}.webm"
        )
        ...
        for series in seris_json:
            ...
            draw.text(
                (1150, 10), f"{hour:02d} H {minute:02d} M", font=font_time, fill=color
            )
...
def getWellVideoPath(agrs):
    try:
        ...
        video_name = (
            video_path + os.path.sep + pd.imagePath + f"_DISH{dishCode}_{well_id}.mp4"
        )
        ...
        download_path = (
            ...
            + os.path.sep
            + pd.imagePath
            + f"_DISH{dishCode}_{well_id}.mp4"
        )
        return 200, jsonify(download_path)
    except:
        return 500, "获取视频路径异常"
...
def queryIncubator():
    try:
            ...
            for dir in dirs:
                ...
                if os.path.isdir(dish_path):
                    if dir[0] == ".":
                        ...
                        dish_json_path = (
                            dish_path + os.path.sep + conf["DISH_STATE_FILENAME"]
                        )
                        ...
                        # 如果关联了则查询该培养箱下面的培养皿是否全部被关联
                            ...
                            dish_code = dir[4:5]
                            dish = dish_mapper.getByIncubatorIdDishCode(
                                incubator.id, dish_code
                            )
                            if not dish:
                                list.append(incubator_name)
        ...
        return 200, jsonify(result_list)
    except:
        return 500, "查询培养箱异常"
                ...
                if os.path.isdir(dish_path):
                    if dir[0] == ".":
                        ...
                        if incubator_name == incubatorName:
                            dish_code = dir[4:5]
                            incubator = incubator_mapper.getByIncubatorCode(
                                incubator_name
                            )
                            if not incubator:
                                list.append(dir)
...
def getCollectionCatalogInfo(agrs):
    ...
    try:
        ...
        result = RestResult(
            code=200, msg="查询采集目录信息成功", count=None, data=catalog.__dict__
        )
