...
def markDistinct(agrs):
    ...
    try:
        ...
        if os.path.exists(imagePath) and dishConf is not None:
            ...
            if (
                serieInfo.outerDiameter
                and serieInfo.innerDiameter
                and serieInfo.outerDiameter > serieInfo.innerDiameter
            ):
                serieInfo.zonaThickness = (
                    serieInfo.outerDiameter - serieInfo.innerDiameter
                ) * 0.425
            ...
            if os.path.exists(jsonPath):
                ...
                if dishJson["avail"] == 1:
                    ...
                    if timeSeries:
                        ...
                        logUtils.info(
                            "dish_state.json文件中 lastEmbryoSerie 与 lastSerie 都为空"
                        )
                        jpgPath = ""
                else:
    ...
    try:
        ...
        for dishMap in dishList:
            ...
            jsonPath = (
                ...
                + imagePath
                + os.path.sep
                + f'DISH{dishMap["dishCode"]}'
                + os.path.sep
                + conf["DISH_STATE_FILENAME"]
            )
            infoMap = dishMap
            infoMap["imagePathShow"] = (
                imagePath[0:4]
                + "-"
                + imagePath[4:6]
                + "-"
