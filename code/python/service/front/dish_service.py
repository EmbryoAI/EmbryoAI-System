...
def querySeriesList(agrs):
    try:
        ...
        # 拼接返回的list
        nginxImageUrl = getdefault(
            conf, "STATIC_NGINX_IMAGE_URL", "http://localhost:80"
        )
        list = []
        ...
        embryo = embryo_mapper.queryByProcedureIdAndCellId(
            procedure_id, current_cell.id
        )
        ...
        seriesResult = SeriesResult(
            200, "OK", list, seris, dishJson["lastSerie"], m_list, last_embryo_serie
        )
...
def queryScrollbarSeriesList(agrs):
    try:
        ...
        seriesResult = SeriesResult(
            200, "OK", list, current_seris, dishJson["lastSerie"], m_list, current_seris
        )
        return 200, jsonify(seriesResult.__dict__)
    except:
